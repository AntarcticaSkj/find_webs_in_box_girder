...
{
    {
        while(1)
        {
            ...
            *mapCloud += *removeBadPointCloud;
            ...
            {
                mapCloud_z_min = minP.z;
                mapCloud_z_max = maxP.z;
            }
            else
            {
                mapCloud_z_min = 1.0 * (runTimes - 1) / runTimes * mapCloud_z_min + 1.0 / runTimes * minP.z;
...
void FindWebs::densityMatchY(MyPointCloudPtr cloud, std::vector<float>& peak_Y)
{
    ...
    // // 寻找密度峰值
    // int avg = cloud->points.size() / dens_c.size();
    ...
    // for (int i = 0; i < dens_c.size(); ++i)
    // {
    //     if (dens_c[i] > peak_judge_thre * avg)
    //     {
    //         if (i - last_peak < 5)
    //         {
    //             last_peak = i;
    ...
    //         if (start_peak > -1)
    //         {
    //             float peak_now = 1.0 * intervalLength * (i + 1 + start_peak) / 2.0 + minP.y;
    //             peak_Y.emplace_back(peak_now);
    //             std::cout << i - start_peak + 1<< "|" ; // peak 所占区间的长度
    //             std::cout << peak_now << " " ;
    //             start_peak = -1;
    //         }
    //     }
    ...
    // // 双指针检测重复区间内的峰，计算scale,  // DE
    // if (peak.size() < 2)
    //     std::cout << "Too few peaks! Set scale to 1.0!\n";
    // else
    // {
    //     int same_time = 0;
    ...
    //     else
    //     {
    //         std::cout << "Can not find enough same peaks! So only two peaks are used to calculate the scale!" << std::endl;
    //         scale_originToTarget = 2000.0 / (peak[1] - peak[0]) / intervalLength;
    //     }
    //     // std::cout << start << " " << end << " " << same_time  << "\n";
    //     std::cout << "Peak from ~ to ~ :" << peak[start] << " " << peak[end] << " , nums = " << same_time + 1  << "\n";
...
void FindWebs::densityMatchX(MyPointCloudPtr cloud, std::vector<float>& peak_x_map)
{
    ...
    for (int i = 0; i < dens_c.size(); ++i)
    {
        if (dens_c[i] > densityMatchX_thre * peak_judge_thre * avg_sum)
        {
    ...
    {
        if (dens_c[i] > 1.5 * peak_judge_thre * avg_sum)
        {
            peak_x_map.emplace_back(1.0 * intervalLength * (i + 1) + minP.x);
            while (i >= 0 && dens_c[i] > 2 * peak_judge_thre * avg_sum )
                --i;
            peak_x_map.emplace_back(1.0 * intervalLength * (i) + minP.x);
            break;
        }
