...
void KD_TREE<PointType>::multi_thread_rebuild()
{
    ...
    while (!terminated)
    {
        ...
        if (Rebuild_Ptr != nullptr)
        {
            ...
            // RE ---------------------------------------
            // if (remove_knn_not_match)
            //     flatten2(*Rebuild_Ptr, Rebuild_PCL_Storage, MULTI_THREAD_REC);
            // else 
...
void KD_TREE<PointType>::Search(KD_TREE_NODE *root, int k_nearest, PointType point, MANUAL_HEAP &q, float max_dist)
{
    ...
    if (!root->point_deleted)
    {
        ...
        if (dist <= max_dist_sqr && (q.size() < k_nearest || dist < q.top().dist))
        {
            ...
            // RE ---------------------------------------
            // root->point_knn_matched = true;
            root->point.data[3] = -1.0;
            // RE ---------------------------------------
...
void KD_TREE<PointType>::flatten(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type)
{
    ...
    // RE -------------------------------------------------
    // 在删除的点中有KNN匹配上的点，加入
    else if (remove_knn_not_match && root->point.data[3] < 0)
    {
        (*MapToUpdate).push_back(root->point);
    }
    // RE -------------------------------------------------
...
// RE ---------------------------------------
// template <typename PointType>
// void KD_TREE<PointType>::flatten2(KD_TREE_NODE *root, PointVector &Storage, delete_point_storage_set storage_type)
// {
//     if (root == nullptr)
//         return;
//     Push_Down(root);
//     if (!root->point_deleted && root->point_knn_matched)
//     {
//         Storage.push_back(root->point);
//     }
//     flatten2(root->left_son_ptr, Storage, storage_type);
//     flatten2(root->right_son_ptr, Storage, storage_type);
//     switch (storage_type)
//     {
//     case NOT_RECORD:
//         break;
...
// RE ---------------------------------------
// template <typename PointType>
// void KD_TREE<PointType>::rebuild_all_and_remove_knn_not_match()
// {
    
//     pthread_mutex_lock(&working_flag_mutex);
//     pthread_mutex_lock(&rebuild_logger_mutex_lock);
//     remove_knn_not_match = true;
...
template <typename PointType>
void KD_TREE<PointType>::updateMapPer(KD_TREE_NODE **root, PointVector& point_cloud)
{
    if (*root == nullptr)
        return;

    if ((*root)->point.data[3] < 0)
        point_cloud.push_back((*root)->point);

    updateMapPer(&((*root)->left_son_ptr),point_cloud);
    updateMapPer(&((*root)->right_son_ptr),point_cloud);
}

template <typename PointType>
void KD_TREE<PointType>::updateMap()
{
    updateMapPer(&Root_Node, *MapToUpdate);
}
// RE ---------------------------------------
